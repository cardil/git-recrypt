...
def helm_repo(tmp_path: Path) -> Path:
    _ = (tmp_path / "Chart.yaml").write_text(
        "apiVersion: v2\nname: myapp\nversion: 0.1.0"
    )
    _ = (tmp_path / "values.yaml").write_text(
        "replicaCount: 1\nimage:\n  tag: latest\n"
    )
    _ = (tmp_path / "values-production.yaml").write_text(
        "db:\n  password: prod-secret\n"
    )
    templates = tmp_path / "templates"
    templates.mkdir()
    _ = (templates / "deployment.yaml").write_text(
        "apiVersion: apps/v1\nkind: Deployment\n"
    )
    _ = (templates / "secret.yaml").write_text(
        "apiVersion: v1\nkind: Secret\ntype: Opaque\nstringData:\n  api-key: abc123\n"
    )
    _ = (templates / "configmap.yaml").write_text("apiVersion: v1\nkind: ConfigMap\n")
    return tmp_path
...
def test_matches_profile_with_kustomization(tmp_path: Path) -> None:
    ...
    _ = (tmp_path / "kustomization.yaml").write_text(
        "resources:\n  - deployment.yaml\n"
    )
    assert KubernetesDetector.matches_profile(tmp_path) is True
...
def test_matches_profile_with_values_and_yamls(tmp_path: Path) -> None:
    ...
    _ = (tmp_path / "values.yaml").write_text("key: value\n")
    for i in range(6):
        _ = (tmp_path / f"file{i}.yaml").write_text(f"item: {i}\n")
    assert KubernetesDetector.matches_profile(tmp_path) is True
...
def test_matches_profile_false_for_generic(tmp_path: Path) -> None:
    ...
    _ = (tmp_path / "README.md").write_text("# Generic repo\n")
    _ = (tmp_path / "main.py").write_text("print('hello')\n")
